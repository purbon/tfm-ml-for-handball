...
from handy.models import normalizer
...
def flush_dataset(aggregation_key, avg_length, n_players=6):
    df = pd.read_hdf(path_or_buf=f"handball.h5", key="pos")
    df = df[df[Schema.ACTIVE_PLAYERS_COLUMN] == n_players]
    ...
    for game, data_points in df.groupby(["GAME", aggregation_key]):
        ...
        normalized_possession = nm.fit(poss_array=data_points, cutoff_size=avg_length, number_of_players=n_players)
        kdf = km.fit(poss_array=normalized_possession)
        flattened_version = normalizer.flatten(poss_array=normalized_possession, number_of_players=n_players)
        cdf = pd.concat((flattened_version, kdf), axis=1)
    ...
    string_fields = ["possession", "game_phases", "tactical_situation", "passive_alert_result", "possession_result",
                     "passive_alert", "offense_type", "throw_zone", "organized_game", "sequence_label"]
    for i in range(5):
    ...
    print("Saving....")
    fl_key = f"dv0_p6-norm{avg_length}" if aggregation_key == "possession" else f"dv0_p6s-norm{avg_length}"
    excel_file = f"handball-vp6" if aggregation_key == "possession" else f"handball-vp6s"
    excel_file = f"{excel_file}-norm-{avg_length}.xlsx"
...
if __name__ == '__main__':
    for aggregation_key in ["possession", "sequence_label"]:
        for avg_length in range(20, 45, 5):
            flush_dataset(aggregation_key=aggregation_key, avg_length=avg_length)
