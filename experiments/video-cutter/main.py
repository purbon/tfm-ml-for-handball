...
if __name__ == '__main__':
    agg_key = "sequence_label"
    for game_id in range(9, 11, 1):
        game = f"game{game_id}"
        folder = f"sgame{game_id}"
        video_path = f"/Users/purbon/Datasets/tfm/videos/p{game_id}/{game}-cut.mp4"
        df = pd.read_csv(f"sgame{game_id}.csv")
        ...
        for index, row in df.iterrows():
            ...
            if start_time < video_file.length():
                ...
                video_file.save_clip(key=key, path=folder)
